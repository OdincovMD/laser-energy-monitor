...
## Как читать прогрев и стабильность

Для приемочной сессии используйте безопасные настройки:

- `Window`: `20`
- `Enter %`: `0.5`
- `Exit %`: `1.0`

Момент прогрева определяется по стабильности каждого источника относительно самого себя. Разные абсолютные значения `BeamGage` и `Ophir` допустимы и не считаются ошибкой.
...
В `measurement-session.RawData.csv` для этого есть колонки:

- `FirstStabilityMetricPercent` - изменение скользящего среднего первого источника;
- `SecondStabilityMetricPercent` - изменение скользящего среднего второго источника;
- `FirstIsStationary` - первый источник стабилизировался;
- `SecondIsStationary` - второй источник стабилизировался;
- `IsStationary` - оба источника стабилизировались.

Момент хорошей стабильности лазера - первая строка, где `IsStationary` становится `1`. В `measurement-session.Events.csv` этому соответствует событие `StationaryEntered`.
