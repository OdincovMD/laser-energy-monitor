...
- Для современного backend проверьте, что зарегистрирован COM ProgID `OphirLMMeasurement.CoLMMeasurement`.
- Для старых устройств Pulsar проверьте, что зарегистрирован x86 ActiveX-контрол `OphirFastX`.
...
- Выберите `Ophir LMMeasurement SDK` для устройств, которые возвращаются вызовом `ScanUSB`.
