...
- Установите подходящий automation package от вендора Ophir.
- Для основного COM backend проверьте, что зарегистрирован ProgID `OphirLMMeasurement.CoLMMeasurement`.
- Для старых устройств Pulsar, которые не возвращаются через `ScanUSB`, дополнительно проверьте x86 ActiveX-контрол `OphirFastX`.
...
- Выберите `Ophir LMMeasurement SDK` как основной COM-вариант для устройств, которые возвращаются вызовом `OphirLMMeasurement.ScanUSB`.
- Выберите `Ophir Pulsar ActiveX (legacy)`, если фирменная утилита видит Pulsar, но `ScanUSB` возвращает ноль устройств.
...
- Предварительная проверка проходит `COM registration` и `COM activation`.
- `ScanUSB` сообщает, что устройств нет.
...
- `ScanUSB` находит хотя бы одно устройство.
...
- Если `ScanUSB` возвращает ноль для видимого Pulsar, возвращает ли `Ophir Pulsar ActiveX (legacy)` ожидаемое количество устройств?
