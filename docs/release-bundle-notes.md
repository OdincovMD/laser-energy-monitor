...
## Рекомендуемые настройки приемочной сессии прогрева

Для проверки момента прогрева лазера использовать:

- `Window`: `20`
- `Enter %`: `0.5`
- `Exit %`: `1.0`

Стабильность определяется внутри каждого источника отдельно. Разные абсолютные значения `BeamGage` и `Ophir` допустимы; приложение смотрит, перестал ли каждый ряд изменяться относительно собственного скользящего среднего.

Каждая строка `measurement-session.RawData.csv` соответствует одному sample одного источника. Межисточниковое временное сопоставление не используется как условие прогрева.

В `measurement-session.RawData.csv` ключевые колонки:

- `FirstStabilityMetricPercent`
- `SecondStabilityMetricPercent`
- `FirstIsStationary`
- `SecondIsStationary`
- `IsStationary`

Момент хорошей стабильности - первая строка с `IsStationary=1`; в `measurement-session.Events.csv` этому соответствует событие `StationaryEntered`.
